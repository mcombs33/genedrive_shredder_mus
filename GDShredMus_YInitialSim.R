...
eFactorial<- expand.grid(Factor1 = seq(0.5, 0.5, length.out=1),   #Development rate
                          ...
                          Factor15 = seq(1.0, 1.0, length.out=1),      #Prob of cutting homologous site
                          Factor16 = seq(0, 0, length.out=1),          #Prob of nonhomologous end joining
                          Factor17 = seq(1, 1, length.out=1),          #Prob of shredding Y chrom
                          Factor18 = seq(0.66, 0.66, length.out=1),    #Prob of XO passing X chrom to offspring
                          Factor19 = seq(0, 0, length.out=1),          #Fitness reduction for gd hom
                          Factor20 = seq(0, 0, length.out=1),          #Fitness reduction for gd het
                          Factor21 = seq(2, 2, length.out=1)           #Polyandry rate
)
vec<-c(1:672)
...
foreach(
  ...
  ##############################################################################
  temp<-(ode(func = Yshred.v2, y = yn, times = 0:(weeks-1), parms = tparms[,i], method = "iteration"))
