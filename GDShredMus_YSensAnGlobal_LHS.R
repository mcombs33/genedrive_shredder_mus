### Title: Script to generate variable parameters for Y-Shredder global sensitivity analysis using latin hypercube sampling
...
#Global Y-Shredder sensitivity analysis
B<-randomLHS(2500, 15) #15 variables, 2500 samples
B2<-B
...
B2[,6]<-qunif(B[,6], min=0.6, max=3.0)         #dispersal kernel rate (0.6 - 3.0)
...
B2[,12]<-qunif(B[,12], min=0.9, max=1.0)       #prob of cutting homologous site
B2[,13]<-qunif(B[,13], min=0.0, max=0.1)       #prob of NHEJ
B2[,14]<-qunif(B[,14], min=0.9, max=1.0)       #prob of shredding Y-chromosome
B2[,15]<-qunif(B[,15], min=0.5, max=1.0)       #prob of XO females passing X
...
write.csv(B2, "YShred_SensAnGlobal_LHSparm.csv", row.names = F)
